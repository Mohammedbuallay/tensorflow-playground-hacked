...
console.log('nerual network:', nn);

//Get the file object from the user and call (ReadDataSet) function.
document.getElementById("csvFileInput").addEventListener('change', ReadDataSet, false);

var reader = new FileReader();

//pass numSample, noise.
function ReadDataSet(evt) {

  //console.log('I am in read data function');
...
function generateData2(file) {

  var dataSet2 = []; //to convert text file to array.
  dataSet2 = file.match(/[^\r\n]+/g);
  //console.log('new dataset format:', dataSet2);

  //Remove the first line (lables).
  let new_line = dataSet2[0].split(",");
  let values = new Array(); //empty array.
  var label = new Array();
  delete dataSet2[0];
  length = new_line.length; //rows length (number of samples).

  //create (length) number of arrays.
  //length-1 (first col is label).
  ...
  dataSet2.forEach(function(line) {
      ...
      for (var i = 0; i < length; i++) {
        if (i!=0) 
        { values[i-1].push(+new_line[i]); } //other cols are the values.
        else { label.push(new_line[i]); } //first col is the label.
      }     
    });
  ...
  if (length > 3)
  {
    ...
    //set values for Tsne.
    const inputElement: HTMLInputElement = document.getElementById('perplexity') as HTMLInputElement
    const perplexity: string = inputElement.value;

    const inputElement2: HTMLInputElement = document.getElementById('learningRateTsne') as HTMLInputElement
    const learningRateTsne: string = inputElement2.value;

    const inputElement3: HTMLInputElement = document.getElementById('MaxIterationsTsne') as HTMLInputElement
    const MaxIterationsTsne: string = inputElement3.value;

    console.log('perplexity', perplexity);
    console.log('learningRateTsne', learningRateTsne);
    console.log('MaxIterationsTsne', MaxIterationsTsne);

    var opt = { 'epsilon': learningRateTsne, 'perplexity': perplexity, 'dim': 2 };
    ...
    for (var i = 0; i < label.length; i++) {
      ...
      for (var j = 0; j < length-1; j++) {
        tmp.push((values[j][i]).toFixed(1));
      };
      pca_data.push(tmp);
    };

    let steps = MaxIterationsTsne;
    
    tsne.initDataRaw(pca_data);
  ...
  const minmaxScaler = new MinMaxScaler({ featureRange: [-6, 6] });
  var final_values = [];
  final_values.push(minmaxScaler.fit_transform(values[0]));
  final_values.push(minmaxScaler.fit_transform(values[1]));

  //console.log('final values',final_values);
  var Mohd_DataSet = [];

  for (var i = 0; i < label.length; i++) {
    let obj = {'label' : label[i], 'x' : final_values[0][i], 'y': final_values[1][i] };
    Mohd_DataSet.push(obj);
  } 
  ...
  trainData = Mohd_DataSet.slice(0,splitIndex),
  testData = Mohd_DataSet.slice(splitIndex), 
...
let btn_perplexity = document.getElementById("perplexity");
const inputElement4: HTMLInputElement = document.getElementById('perplexity') as HTMLInputElement;
btn_perplexity.addEventListener("change", (e:Event) => setPerplexity(inputElement4.value));

function setPerplexity(val) {
  console.log(val);
  (document.getElementById("valuePerplexity") as HTMLInputElement).innerHTML = val;
}


let btn_learningRateTsne = document.getElementById("learningRateTsne");
const inputElement5: HTMLInputElement = document.getElementById('learningRateTsne') as HTMLInputElement;
btn_learningRateTsne.addEventListener("change", (e:Event) => setLearningRateTsne(inputElement5.value));

function setLearningRateTsne(val) {
  console.log(val);
  (document.getElementById("valuelearningRateTsne") as HTMLInputElement).innerHTML = val;
}

let btn_MaxIterationsTsne = document.getElementById("MaxIterationsTsne");
const inputElement6: HTMLInputElement = document.getElementById('MaxIterationsTsne') as HTMLInputElement;
btn_MaxIterationsTsne.addEventListener("change", (e:Event) => setMaxIterationsTsne(inputElement6.value));

function setMaxIterationsTsne(val) {
  console.log(val);
  (document.getElementById("valueMaxIterationsTsne") as HTMLInputElement).innerHTML = val;
}